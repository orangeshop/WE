...
interface LocationAndDateProps {
  initialCalendarValue: Value;
  initialWeddingHall: string;
...
const LocationAndDate = forwardRef(
  (
    {
    ...
    const [calendarValue, setCalendarValue] =
      useState<Value>(initialCalendarValue);
    const [modalIsOpen, setModalIsOpen] = useState<boolean>(false);
    const [timeDay, setTimeDay] = useState<string>(initialTime.day);
    const [timeHour, setTimeHour] = useState<number>(initialTime.hour);
    const [timeMinute, setTimeMinute] = useState<number>(initialTime.minute);
    const [wedding_hall, setWeddingHall] = useState<string>(initialWeddingHall);
    const [address_detail, setAddressDetail] =
      useState<string>(initialAddressDetail);
    ...
    useEffect(() => {
      ...
      setWeddingHall(initialWeddingHall);
      setAddressDetail(initialAddressDetail);
      setAddress(initialAddress);
      setLatitude(initialLatitude);
      setLongitude(initialLongitude);
      setTimeDay(initialTime.day);
      setTimeHour(initialTime.hour);
      setTimeMinute(initialTime.minute);
    }, [
      initialCalendarValue,
      initialWeddingHall,
      initialAddressDetail,
    ...
    const handleSubmit = async () => {
      ...
      const hour = timeHour;
      const minute = timeMinute === 0 ? 0 : 30;
      ...
      <div>
        ...
        <div className="w-full flex justify-between gap-3 mb-3">
          ...
          <select
            ...
            name="time_minute"
            value={timeMinute}
            onChange={(e) => setTimeMinute(parseInt(e.target.value, 10))}
            className={`w-full px-2 py-2 border text-md border-gray-400 focus:border-gray-400 text-center bg-white ${
